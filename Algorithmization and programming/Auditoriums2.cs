...
{
    static bool IsInt(string try_int)
    {
        bool check_n = true;
    ...
    static int TryStringToInt(string phrase)
    {
        bool check = false;
        string vvod;
        int result = -1;
    ...
    static bool Try2optionsTF(string phrase, string optionT, string optionF)
    {
        bool check = false;
        string vvod;
        bool result = false;
    ...
    static void Add(ref ArrayList Auds)
    {
        Console.Clear();
    ...
    static void Change(ref ArrayList Auds)
    {
        Console.Clear();
    ...
    static void SearchMenu(ref ArrayList Auds)
    {
        Console.Clear();
        ...
        else if (searchOption == "5") { ShowMenu(ref Auds); }
        else { SearchMenu(ref Auds); }
    }
    public static void ShowMenu(ref ArrayList Auds)
    {
        Console.Clear();
        ...
        if (option == "1") { Add(ref Auds); ShowMenu(ref Auds); }
        else if (option == "2")
        {
            bool Out = false;
            while (Out == false)
            {
                Add(ref Auds);
                if (Try2optionsTF("Do you want to continue?", "Yes", "No") == false) { Out = true; ShowMenu(ref Auds); }
            }
        }
        else if (option == "3") { Change(ref Auds); }
        else if (option == "4")
        {
            Console.Clear();
        ...
        else if (option == "5") { SearchMenu(ref Auds); }
        else if (option == "6")
        {
            Console.Clear();
            Environment.Exit(0);
        }
        else { ShowMenu(ref Auds); }
    }
}
class Program
{
